...
# define regions
g.region n=$N s=$S e=$E w=$W res=0:05 save=rgn_05min --overwrite
g.region n=$N s=$S e=$E w=$W res=0:15 save=rgn_15min --overwrite
g.region n=$N s=$S e=$E w=$W res=0:00:30 save=rgn_30sec --overwrite
...
for RGN in 05min 15min 30sec
do    
    g.region region=rgn_${RGN}
    r.resamp.stats \
	input=merit_slope \
	output=merit_slope_resamp_${RGN} \
	method=average \
	--overwrite

    r.out.gdal \
	input=merit_slope_resamp_${RGN} \
	output=${AUX_DATADIR}/merit_slope_${RGN}.tif \
	--overwrite
done
...
# do
#     r.in.gdal \
# 	input=${AUX_DATADIR}/${TARGET_RES}_${VAR}_subrgn.tif 
# 	output=${TARGET_RES}_${VAR}  \
# 	--overwrite
# done
...
# r.out.gdal\
#     input=merit_dir \
#     output=${AUX_DATADIR}/merit_rgn_dir.tif \
#     type=Byte \
#     nodata=247 \
#     --overwrite

# # Set region to coarse model resolution
# g.region raster=merit_elv res=0:06
# g.region -p
